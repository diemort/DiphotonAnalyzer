...
void plot_limits_aqgc( const char* path = "/afs/cern.ch/work/l/lforthom/private/twophoton/mcprod/fpmc/build/scan_result.dat" )
{
  ...
  //const double xsec_limit = 1.6995e-3; //FIXME
  //const double xsec_limit = 3.02632/(0.2858*0.7*0.9*(4.291753570355+1.438424638452+3.625791461729)*1e3);
  const double xsec_limit = 2.82413/(0.3754*0.7*0.9*(4.291753570355+1.438424638452+3.625791461729)*1e3); //may25
  const double xsec_sm = 4.2054e-7;
  cout << "xsection limit: " << xsec_limit << endl;
  //const double sel_eff = 0.415; // backup (xi in 0.02/0.15)
  //const double sel_eff = 0.5721; // tighter xi range!
  //const double sel_eff = 0.604; // tighter xi range! (may20)
  const double sel_eff = 0.6378; // tighter xi range! (may25)
  ifstream file( path );
  //TGraph2D gr_xs( path, "%lg\t%lg\t%lg" );
  TGraph2D gr_xs, gr_xs_norm;
  ...
  while ( file >> z1 >> z2 >> xs ) {
    ...
    gr_xs.SetPoint( gr_xs.GetN(), z1, z2, xs*sel_eff );
    gr_xs_norm.SetPoint( gr_xs.GetN(), z1, z2, xs/xsec_sm );
    if ( fabs( z1 ) < 1.e-6 )
      gr_xs_z2.SetPoint( gr_xs_z2.GetN(), z2, xs*sel_eff );
    if ( fabs( z2 ) < 1.e-6 )
      gr_xs_z1.SetPoint( gr_xs_z1.GetN(), z1, xs*sel_eff );
  }
  ...
  {
    Canvas c( "limits_aqgc_1d", top_label, "CMS-TOTEM", "Simulation preliminary" );
    TMultiGraph mg;
  ...
  {
    Canvas c( "limits_aqgc_2d", "9.4 fb^{-1} (13 TeV)", "CMS-TOTEM", "Preliminary", false, Canvas::Align::right );
    gr_cont->SetTitle( ";#zeta_{1} (#times 10^{-12}) (GeV^{-4});#zeta_{2} (#times 10^{-12}) (GeV^{-4})" );
    ...
    c.AddLegendEntry( gr_cont, "95% confidence region", "l" );
    c.AddLegendEntry( &gr_1d, "1-D limit, 95% confidence region", "l" );
  ...
  {
    Canvas c( "scan_xsec_aqgc_norm", top_label, "CMS-TOTEM", "Simulation preliminary" );
    c.SetRightMargin( 0.15 );
    gr_xs_norm.Draw( "cont4z" );
    gr_xs_norm.SetTitle( ";#zeta_{1} (#times 10^{-12});#zeta_{2} (#times 10^{-12});#sigma_{obs}/#sigma_{SM}" );
    c.Prettify( (TH1D*)gr_xs_norm.GetHistogram() );
    c.SetLogz();
    c.SetGrid();
    c.Save( "pdf,png", OUT_PATH );
  }
  {
    Canvas c( "scan_xsec_aqgc", top_label, "CMS-TOTEM", "Simulation preliminary" );
    c.SetRightMargin( 0.15 );
    //gr_xs.Draw( "cont4z" );
    gr_xs.Draw( "cont4z" );
    //gr_xs.Draw( "colz" );
