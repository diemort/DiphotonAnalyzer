...
//    delete f_in;
  ...
  //const double probed_coupl = -0.301;
  const double probed_coupl = -1.;
  //const double probed_coupl = -1.5;

  //retrieved from /afs/cern.ch/user/j/juwillia/public/forLaurent/AC_microAOD
  vector<sample_t> samples = {
    { 500.,  -0.301, "samples/ntuple-alp-m500_f5e-1.root",    5.884e-2, 1.504e-1, 5000 },
    { 500.,  -1.,    "samples/ntuple-alp-m500_f1e-1.root",    2.310e-3, 5.912e-3, 5000 },
    { 500.,  -1.5,   "samples/ntuple-alp-m500_f1e-1p5.root",  2.297e-4, 8.623e-4, 5000 },
    { 750.,  -0.301, "samples/ntuple-alp-m750_f5e-1.root",    5.015e-2, 7.633e-2, 5000 },
    { 750.,  -1.,    "samples/ntuple-alp-m750_f1e-1.root",    1.978e-3, 3.116e-3, 5000 },
    { 750.,  -1.5,   "samples/ntuple-alp-m750_f1e-1p5.root",  2.001e-4, 3.809e-4, 5000 },
    { 1000., -0.301, "samples/ntuple-alp-m1000_f5e-1.root",   2.382e-2, 4.377e-2, 5000 },
    { 1000., -1.,    "samples/ntuple-alp-m1000_f1e-1.root",   9.534e-4, 1.745e-3, 5000 },
    { 1000., -1.5,   "samples/ntuple-alp-m1000_f1e-1p5.root", 9.595e-5, 1.466e-4, 5000 },
    { 1250., -0.301, "samples/ntuple-alp-m1250_f5e-1.root",   1.089e-2, 3.062e-2, 5000 },
    { 1250., -1.,    "samples/ntuple-alp-m1250_f1e-1.root",   4.624e-4, 1.282e-3, 5000 },
    { 1250., -1.5,   "samples/ntuple-alp-m1250_f1e-1p5.root", 4.654e-5, 1.163e-4, 5000 },
    { 1500., -0.301, "samples/ntuple-alp-m1500_f5e-1.root",   4.823e-3, 2.015e-2, 5000 },
    { 1500., -1.,    "samples/ntuple-alp-m1500_f1e-1.root",   1.968e-4, 7.331e-4, 5000 },
    { 1500., -1.5,   "samples/ntuple-alp-m1500_f1e-1p5.root", 1.968e-5, 7.123e-5, 5000 },
    { 1750., -0.301, "samples/ntuple-alp-m1750_f5e-1.root",   1.504e-3, 1.214e-2, 5000 },
    { 1750., -1.,    "samples/ntuple-alp-m1750_f1e-1.root",   5.819e-5, 5.556e-4, 5000 },
    { 1750., -1.5,   "samples/ntuple-alp-m1750_f1e-1p5.root", 5.799e-6, 5.155e-5, 5000 },
    { 2000., -0.301, "samples/ntuple-alp-m2000_f5e-1.root",   2.162e-4, 8.872e-3, 5000 },
    { 2000., -1.,    "samples/ntuple-alp-m2000_f1e-1.root",   3.777e-7, 3.344e-4, 5000 },
    { 2000., -1.5,   "samples/ntuple-alp-m2000_f1e-1p5.root", 3.552e-9, 3.346e-5, 5000 },
  };
  ...
  TGraph2D g_el_eff, g_el_acc, g_el_xs;
  gggg::TreeEvent ev;
  ...
  for ( const auto& s : samples ) {
    ...
    g_el_acc.SetPoint( i, s.m, s.k, el_acc );
    g_el_xs.SetPoint( i, s.m, s.k, s.xsec_bare );
    g_el_vals.SetPoint( i, s.m, s.k );
  ...
  //--- prepare the data cards for limits setting
  {
    ...
    for ( auto& h_bm : v_h_binned_mass ) {
      ...
      hist->SetLineColor( Plotter::colour_pool[idx] );
      hist->SetLineWidth( 3 ) ;
      hs.Add( hist );
      ++idx;
    }
    ...
    PaveText::topLabel( Form( "f^{-1} = 10^{%g} GeV^{-1}", probed_coupl ) );
  ...
  {
    Canvas c( "elastic_xsec_scan_alp", "FPMC #gamma#gamma #rightarrow #gamma#gamma + ALP", "Simulation preliminary" );
    c.SetRightMargin( 0.15 );
    g_el_xs.SetTitle( ";m_{a} (GeV);log(f^{-1}/GeV^{-1});Signal cross section (pb)" );
    g_el_vals.SetMarkerStyle( 20 );
    g_el_xs.Draw( "colz" );
    g_el_vals.Draw( "p same" );
    g_el_vals.SetMarkerStyle( 3 );
    g_el_xs.GetZaxis()->SetTitleOffset( 1.4 );
    c.SetLogz();
...
/*      TFitResultPtr fit_res = g.second.Fit( &f_fit, "s0+" );
      ...
      c.AddLegendEntry( &g.second, Form( "f^{-1} = 10^{%g} GeV^{-1}", g.first ), "ep" );
      ++i;
    }
    mg.Draw( "ap" );
  ...
  for ( auto& p : plots ) {
    ...
    for ( unsigned short j = 0; j < num_samples; ++j ) {
      ...
      p.second[j].SetFillColorAlpha( Plotter::colour_pool[j/3], 0.15+(j%3)*0.2 );
